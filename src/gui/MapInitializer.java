...
public class MapInitializer implements Serializable {
    ...
    private Set<PortButton<Harbour>> harboursButtons;
    ...
    public void getHarbours() {
        ...
        for (int i = 0; i < buttons.getLength(); i++) {
            ...
            list.add(btn);
            landConnections.put(element.getAttribute("name"), getLandConnections());
            routes.add(getMapOfRoutesAsStrings());
            set.add(btn);
            seaPorts.put(element.getAttribute("name"), btn.getModel());
        }
    ...
    private synchronized Harbour gatherHarbour(Point2D location) {
        ...
        h.setName(element.getAttribute("name"));
        seaPorts.put(element.getAttribute("name"), h);
        return h;
    }
    ...
    private Map<String, String[]> getMapOfRoutesAsStrings() {
        NodeList nodes = element.getElementsByTagName("connection");
    ...
    private Set<String> getLandConnections() {
        NodeList nodes = element.getElementsByTagName("land");
    ...
    public synchronized void getCivilianAirports() {
        ...
        for (int i = 0; i < buttons.getLength(); i++) {
            ...
            landConnections.put(element.getAttribute("name"), getLandConnections());
            routes.add(getMapOfRoutesAsStrings());
            airPorts.put(element.getAttribute("name"), btn.getModel());
            set.add(btn);
        }
    ...
    private CivilianAirport gatherCivilianAirport(Point2D location) {
        ...
        airPorts.put(element.getAttribute("name"), c);
        return c;
    }
    ...
    public synchronized void getMilitaryAirports() {
        ...
        for (int i = 0; i < buttons.getLength(); i++) {
            ...
            list.add(btn);
            routes.add(getMapOfRoutesAsStrings());
            mAirPorts.put(element.getAttribute("name"), btn.getModel());
            set.add(btn);
        }
    ...
    private MilitaryAirport gatherMilitaryAirport(Point2D location) {
        ...
        c.setName(element.getAttribute("name"));
        mAirPorts.put(element.getAttribute("name"), c);
        return c;
    }
     ...
     * @throws MapInitException information of issues during initialization
     */
    public Map<Port, Map<Port, List<Cross>>> getConnectionForAircraftCarrier() throws MapInitException {
        ...
        for (int i = 0; i < conns.getLength(); i++) {
            ...
            if (portFrom == null) {
                throw new MapInitException("Specific Harbour does not exist!",
                        MapInitException.ErrorType.FILE_NOT_COHERENT);
            }

            if (portTo == null) {
                throw new MapInitException("Specific Military airport does not exist!",
                        MapInitException.ErrorType.FILE_NOT_COHERENT);
            }
     ...
     */
    public Map<String, MilitaryAirport> getMAirPorts() {
        return mAirPorts;
    }
     ...
     */
    public Set<PortButton<Harbour>> getHarboursButtons() {
        return harboursButtons;
    }
     ...
     */
    public Set<PortButton<CivilianAirport>> getcAirportButtons() {
        return cAirportButtons;
    }
     ...
     */
    public Set<PortButton<MilitaryAirport>> getmAirportButtons() {
        return mAirportButtons;
    }
}
